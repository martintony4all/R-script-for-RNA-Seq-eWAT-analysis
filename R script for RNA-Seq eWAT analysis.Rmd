...
result.pvclust <- pvclust(na.omit(ReadAggExp[,-1]), nboot = 100)
...
# do MDS analysis
mds.result <- cmdscale(1-cor(ReadAggExp[,-(1)],method="spearman"), k = 2, eig = FALSE, add = FALSE, x.ret = FALSE)

x <- mds.result[, 1]
y <- mds.result[, 2]

my.pch.organ <- c(rep(16,2), rep(17,2))
...
# Focus of phygocytic eWAT on young mouse
my.young_mice.data <- ReadAggExp[,c(2,3)]
mds.young_mice_result <- cmdscale(1-cor(my.young_mice.data,method="spearman"), k = 1, eig = FALSE, add = FALSE, x.ret = FALSE)
x <- mds.result[, 1]
y <- mds.result[, 2]

my.colors <- c(rep("coral",1))

pdf(paste(Sys.Date(),"GSE154832_young_mice_MDS_plot.pdf",sep="_"))
plot(x, y, xlab = "MDS dimension 1", ylab = "MDS dimension 2",main="Multi-dimensional Scaling",cex=2, pch=16,col=my.colors)
legend("top", c("2m"), col = c("coral"), pch = 16, bty = 'n', pt.cex = 2)
dev.off()

# Focus of phygocytic eWAT on aged mouse
my.aged.mice <- ReadAggExp[,c(4,5)]

# do MDS analysis
mds.aged.result <- cmdscale(1-cor(ReadAggExp[,c(4,5)],method="spearman"), k = 1, eig = FALSE, add = FALSE, x.ret = FALSE)
x <- mds.result[, 1]
y <- mds.result[, 2]

my.colors <- c(rep("coral",1))

pdf(paste(Sys.Date(),"GSE154832_aged_mice_MDS_plot.pdf",sep="_"))
plot(x, y, xlab = "MDS dimension 1", ylab = "MDS dimension 2",main="Multi-dimensional Scaling",cex=2, pch=16,col=my.colors)
legend("topright",c("20m"),col=c("coral"),pch=16,bty='n',pt.cex=2)
dev.off()
...
#writing deferentially expressed genes to a csv file
...
# GO-BP Gene Set Enrichment Analysis
...
gseaplot(ego2, geneSetID = "GO:0003676", title = "GO:0003676 nucleic acid binding")
gseaplot(ego2, geneSetID = "GO:0006351", title = "GO:0006351 mitochondrion organization")
gseaplot(ego2, geneSetID = "GO:0006355", title = "GO:0006355 regulation of transcription, DNA-templated")
gseaplot(ego2, geneSetID = "GO:0006793", title = "GO:0006793 phosphorus metabolic process")
gseaplot(ego2, geneSetID = "GO:0006796", title = "GO:0006796 phosphate-containing compound metabolic process")
gseaplot(ego2, geneSetID = "GO:0009653", title = "GO:0009653 anatomical structures morphogenesis")


# plot dotplot with no sign information
pdf(paste(Sys.Date(),"GSEA_dotplot.pdf", sep = "_"))
dotplot(ego2,  title = "Significant GO ALL terms")
dev.off()
...
cnetplot(ego2, categorySize="pvalue", foldChange=res.geneList) #categorySize can be scaled by 'pvalue' or 'geneNumber
dev.off()

#cnetplot of GO-BP intereaction network
pdf(paste(Sys.Date(),"GSEA_BP_cnetplot.pdf", sep = "_"), width = 10)
cnetplot(ego2.1, categorySize="pvalue", foldChange=res.geneList) #categorySize can be scaled by 'pvalue' or 'geneNumber
dev.off()

#cnetplot of GO-CC intereaction network
pdf(paste(Sys.Date(),"GSEA_CC_cnetplot.pdf", sep = "_"), width = 10)
cnetplot(ego2.2, categorySize="pvalue", foldChange=res.geneList) #categorySize can be scaled by 'pvalue' or 'geneNumber
dev.off()

#cnetplot of GO-MF intereaction network
pdf(paste(Sys.Date(),"GSEA_MF_cnetplot.pdf", sep = "_"), width = 10)
cnetplot(ego2.3, categorySize="pvalue", foldChange=res.geneList) #categorySize can be scaled by 'pvalue' or 'geneNumber
dev.off()
...
# GSEA plot of interesting pathways in KEGG
...
# plot dotplot with no sign information
pdf(paste(Sys.Date(),"KEGG_GSEA_dotplot.pdf", sep = "_"))
dotplot(kk2,  title = "Significant KEGG pathways")
dev.off()
...
cnetplot(kk2, categorySize="pvalue", foldChange=res.geneList) #categorySize can be scaled by 'pvalue' or 'geneNumber
dev.off()
...
pheatmap(ReadAggExp2[genes.eWAT,],
         cluster_cols = F,
         cluster_rows = T,
